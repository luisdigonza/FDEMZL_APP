<?php 
    ...
    if(isset($_POST['insert_brand_title'])){
        ...
        if($numOfResults>0){
            echo "<script>alert('La etiqueta ya existe en la base de datos');</script>";
        }else{
                $insert_query="INSERT INTO `brands` (brand_title) VALUES ('$brand_title')";
                $insert_result=mysqli_query($con,$insert_query);
                if ($insert_result){
                    echo "<script>alert('La etiqueta ha sido añadida exitosamente');</script>";
                }
...
<form action="" method="POST" class="mb-2">
    <div class="input-group w-90 mb-3">
        ...
        <input type="text" class="form-control" name="brand_title" placeholder="Añadir Etiquetas" aria-label="Brands" aria-describedby="basic-addon1">
    </div>
    <div class="input-group w-10 mb-3">
        <input type="submit" class="btn btn-primary" name="insert_brand_title" value="Añadir Etiquetas" aria-label="Brands" aria-describedby="basic-addon1">
    </div>
</form>