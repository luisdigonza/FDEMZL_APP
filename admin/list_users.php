        ...
        <div class="table-data">
            <table class="table table-bordered table-hover table-striped text-center">
                <thead class="table-dark">
                    <?php
                    $get_user_query = "SELECT * FROM `admin_table`";
                ...
                <tbody>
                    ...
                    // $row_count = mysqli_num_rows($get_user_result);
                    if ($row_count == 0) {
                        echo "<h2 class='text-center text-light p-2 bg-dark'>No existen usuarios</h2>";
                    } else {
                        $id_number = 1;
                        while ($row_fetch_users = mysqli_fetch_array($get_user_result)) {
                            $user_id = $row_fetch_users['admin_id'];
                            $username = $row_fetch_users['admin_name'];
                            $user_email = $row_fetch_users['admin_email'];
                            $user_image = $row_fetch_users['admin_image'];
                            echo "
                            <tr>
                            <td>$id_number</td>
                            <td>$username</td>
                            <td>$user_email</td>
                            <td>
                                <img src='../admin/admin_images/$user_image' alt='$username photo' class='img-thumbnail' width='100px'/>
                            </td>
                            <td>
                                        ...
                                        <div class='modal-content' style='width:80%;'>
                                            <div class='modal-body'>
                                                <div class='d-flex flex-column gap-3 align-items-center text-center'>
                                                    ...
                                                    <h2>Estas seguro?</h2>
                                                    <p>
                                                        ¿Está seguro de que desea eliminar este registro? Este proceso no se puede retroceder.
                                                    </p>
                                                    <div class='btns d-flex gap-3'>
                                                        ...
                                                        <button type='button' class='btn px-5 btn-primary' data-bs-dismiss='modal'>
                                                            <a class='text-light' href='index.php?delete_user=$user_id'>
                                                                Eliminar <strong>$username</strong>
                                                            </a>
                                                        </button>
                                                    </div>
                                                </div>
